...
async function verifyFiltering() {
	...
	console.log(
		`info: Found ${toolCount} tools with filter "Orders API,Customers API"`,
	);

	// We expect ~19 tools (11 orders + 8 customers, roughly)
	if (toolCount > 0 && toolCount < 50) {
		console.log(
			`✅ Filter Logic Passed: Tool count ${toolCount} is within expected range (<50).`,
		);
	} else {
		console.error(
			`❌ Filter Logic Failed: Tool count ${toolCount} is unexpected.`,
		);
		process.exit(1);
	}
	...
	const hasCustomers = toolsResult.tools.some((t) =>
