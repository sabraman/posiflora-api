...
async function verifyCompliance() {
    ...
    try {
        ...
        // @ts-ignore - listResourceTemplates might be missing in strict types depending on version, checking listResources first
        ...
        const staticResource = resources.resources.find(r => r.uri === "posiflora://openapi.json");
        if (staticResource) console.log("✅ Found Static Resource: posiflora://openapi.json");
        ...
        // @ts-ignore
        const templatesResult = await client.request({ method: "resources/templates/list" }, SimpleTemplateSchema);
        // @ts-ignore
        ...
        try {
            ...
            const tool = tools.tools.find(t => t.name.includes("get") && t.name.includes("customer"));
            ...
            if (tool) {
                ...
                try {
                    ...
                    console.error("❌ Tool call succeeded unexpectedly (Should have failed with 401)");
                    console.log("   Result content:", JSON.stringify(result, null, 2));
                } catch (e: any) {
                    // If it failed here, good!
                    throw e;
                }
            } else {
            ...
            try {
                await client.callTool({
                    name: "get_v1_customers_id", // Assuming this exists or similar
                    arguments: { id: "NON_EXISTENT_ID_12345" }
                });
                console.error("❌ Invalid Resource Call succeeded (Should have failed with 404)");
            } catch (e: any) {
                console.log(`   Received Error from Invalid Call: ${e.message}`);
                if (e.message.includes("404") || e.code === ErrorCode.InvalidRequest) {
                    console.log("✅ Error Handling Validation Passed (404 mapped correctly)");
                } else {
            ...
            // Check error code
            // SDK might wrap error.
            console.log(`   Received Error: ${e.message}`);
            // We want to see if the error code matches InvalidRequest (-32600) or similar? 
            // McpError codes: InvalidRequest = -32600, MethodNotFound = -32601, InvalidParams = -32602, InternalError = -32603
            // In our server we mapped 401 to InvalidRequest.

            if (e.code === ErrorCode.InvalidRequest || e.message.includes("InvalidRequest") || e.message.includes("401")) {
                console.log("✅ Error Handling Validation Passed (401 mapped correctly)");
            } else {
                // It might be generic InternalError if our logic failed
                console.log(`⚠ Received Error Code: ${e.code}. Verifying if this is acceptable...`);
                if (e.code === ErrorCode.InternalError) {
                    console.error("❌ Got InternalError (-32603) instead of mapped code. Compliance fix might be incomplete.");
                } else {
                    console.log("✅ Error seems structured.");
                }
            }
        }
