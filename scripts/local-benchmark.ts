...
async function runBenchmark() {
    ...
    if (toolCount > 100) {
        console.log(`✅ Tool Discovery: PASS (${toolCount} tools loaded in ${listTime.toFixed(2)}ms)`);
    ...
    const customerTool = toolsResult.tools.find(t => t.name.includes("customers"));
    ...
    const openApiResource = resourcesResult.resources.find(r => r.uri === "posiflora://openapi.json");
    if (openApiResource) {
        console.log("✅ OpenAPI Resource Found");
        // Try reading it
        const resourceContent = await client.readResource({ uri: "posiflora://openapi.json" });
        // @ts-ignore - The types for SDK might be strict unions, simplfying check
        const firstContent = resourceContent.contents?.[0];
        if (firstContent && 'text' in firstContent && typeof firstContent.text === 'string' && firstContent.text.length > 100) {
            console.log(`✅ OpenAPI Resource Read Success (${firstContent.text.length} bytes)`);
    ...
    if (!process.env.POSIFLORA_API_KEY) {
        ...
        try {
            ...
            const customerTools = toolsResult.tools.filter(t => t.name.includes("customers") && t.name.includes("get"));
